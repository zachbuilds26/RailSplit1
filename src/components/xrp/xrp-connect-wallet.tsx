...
import { buildFaucetUrl, getRail } from "@/lib/chain";
...
export function XrpConnectWallet({ compact = false }: { compact?: boolean }) {
  ...
  const wrongChain = isConnected && chainId !== rail.chain.id;
    ...
    try {
      ...
      setSwitchFailure(
        /user rejected|user denied/i.test(message)
          ...
          : message || `RailSplit could not switch to ${rail.chain.name}.`,
      );
    }
  }
      ...
      <div className={compact ? "" : "w-full"}>
        <div className="border border-warning/30 bg-warning/10 p-4">
          ...
          <p className="mt-2 text-xs leading-5 text-muted">
            RailSplit runs on {rail.chain.name}. Switch networks to continue.
          </p>
          <button
            type="button"
          ...
          >
            {isSwitching ? "Switching…" : `Switch to ${rail.chain.name}`}
          </button>
          {switchFailure && (
            <p role="alert" aria-live="assertive" className="mt-3 text-xs leading-5 text-danger">
              {switchFailure}
