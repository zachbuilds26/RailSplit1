...
async function main() {
  ...
  // The deploy transaction's own receipt carries the block. The event scan in
  // the dashboard starts here so it never walks the chain from genesis.
  const deployReceipt = await provider.getTransactionReceipt(deployTx.hash);
  const deployBlock = deployReceipt ? deployReceipt.blockNumber : 0;
